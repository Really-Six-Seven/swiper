...
def regret(user):
    ...
    if regret_times < config.REGRET_TIMES:
        regret_times += 1
        now_second = 96400-now_time.hour*3600-now_time.minute*60-now_time.second
        cache.set(key,regret_times,now_second)

        record=Swipe.objects.filter(uid=user.id).latset('stime')
